...
- 当前消息、原帖、楼层评论、直接回复对象和压缩摘要统一显示“昵称 + UID”；API 偶发缺少 UID 时保留原文并改用按评论/通知/帖子隔离的本地身份锚点，不会把不同匿名发言人合并；
- 本地备用身份不是小黑盒 UID，只用于内容归属和 AstrBot 非空发送者 ID，不能获得主人/管理员权限或通过 UID 白名单；白名单模式仍对缺 UID 事件安全拒绝；
- 每张图片都有由本地代码生成的来源、所有者和身份锚点。原帖图片归原帖作者，当前评论图片归当前发送者；只有所有者 UID 完全匹配时，模型才能称为“你发的图”；
- 长楼层 LLM 压缩使用“整体主题 + 逐人发言摘要”，模型只选择程序分配的 `speaker_key`；任一编造、交换或格式异常会拒绝整份压缩结果并回退原有有界上下文，不会静默丢掉部分发言；
- 24 小时视觉快照保存并校验所有者 UID、昵称和角色，内存缓存按帖子隔离；同帖作者 UID 暂缺时可凭帖子锚点和图片指纹复用，旧版无昵称/角色快照或跨帖子/跨已知作者命中不会复用；
...
- 辅助图片/上下文 Provider 失败后进入短时冷却，避免每条评论重复 403、超时或异常；管理页和日志都会提示冷却状态，管理页可手动结束冷却，主对话 Provider 不会因此被插件熔断；
- 帖子/楼层网络上下文继续使用 60 秒有界缓存和 single-flight；这与 24 小时视觉文字快照相互独立；
...
- 楼层回复使用当前消息/直接回复对象优先的焦点路由，并支持按来源语义压缩长原帖、楼层与图片背景；
...
| `context.thread_reply_compressed_image_chars` | `800` | 被动回复中缓存/新识别原帖图片描述的硬上限 |
...
v1.2.13 对超过 2400 字的可压缩背景默认额外调用一次上下文 Provider：输入仍把原帖和楼层分开，
原帖输入最多 8000 字、最近楼层输入合计最多 8000 字（并继续受全局读取上限约束）；输出分别硬限
700 / 1400 字，并单独给出“当前局部话题”和“与原帖关系”。当前消息与直接回复对象只作为压缩
参考，最终上下文中仍使用未经摘要替换的原文。

v1.2.14 在压缩输入中额外加入由本地代码从实际楼层评论提取的只读参与者列表，并修复该列表在
`on_llm_request` 二次转换时被漏掉的问题；昵称与 UID 会在压缩完成后再次作为身份锚点附回最终
临时上下文。因此即使较小的压缩模型把楼层摘要写成“某用户”，主模型仍能看到本轮所选最近楼层
中真实的“昵称 ↔ UID”映射。身份锚点仍属于不可信社区背景，不会修改 AstrBot system prompt
或人格。图片/上下文辅助 Provider 连续失败时会短暂冷却并改走备用或确定性兜底；冷却只作用于
这些额外预处理调用，不会阻止 AstrBot 主 Agent 自己的 Provider/回退链路。

v1.2.17 将参与者列表改为“程序分配的 `speaker_key` → 本地昵称/UID/备用锚点”映射，压缩模型
不再直接改写身份文本；只要任一身份键、摘要类型或身份项格式异常，整份压缩结果都会被拒绝并
回到上述确定性路径，不会只丢掉错误条目后继续使用残缺摘要。小黑盒没有返回 UID 时，当前消息、
楼层评论和直接回复对象仍保留原文，并以稳定评论/通知锚点彼此分隔；该锚点不参与任何 UID 权限判断。

v1.2.15 会在机器人主动评论带图帖子时保存一份 24 小时的原帖视觉快照，并把真实发送后返回的
机器人评论 ID 与该快照关联。后续评论若回复该机器人楼层，优先按直接回复目标/根评论恢复原快照；
接口再次提供原图时也可按去除查询参数后的有序图片指纹命中。缓存描述会作为独立低优先级字段
参与上述楼层语义压缩，图片内容、评论文字和昵称/UID 仍保持不同来源。快照只保存文字与脱敏元数据，
最多 512 条常用描述驻留内存，SQLite 记录到期后由日常清理回收。

短楼层、显式关闭语义压缩、超时、Provider 异常或结果无法解析时，立即退回 v1.2.12 的确定性
路径：原帖默认 1600 字、最近楼层默认 12 条且单条最多 800 字。通知中的直接回复对象在两条路径
里都会单独保留，并从普通楼层窗口去重。可信焦点规则最后注入；当前消息能独立理解时不得为了
迎合原帖强行关联。主动刷帖完全不走这套被动楼层语义压缩，仍按 `max_post_chars` /
`max_thread_comments` 读取原帖和辅助评论。
...
- UID 全部按字符串处理；
- API 未提供 UID 时保留消息并使用评论/通知级本地身份锚点；该锚点不是 UID，不能命中主人、管理员或 UID 黑白名单，白名单模式会安全拒绝；
...
- v1.2.17 数据库迁移版本为 **v10**：视觉快照新增所有者 UID、昵称和角色；同帖作者 UID 暂缺时可按帖子锚点与图片指纹安全复用，旧记录原位保留，但没有昵称/角色等可靠归属的旧图片描述不会被复用；
...
`daily_counters`、`notification_cursors`、`notification_backfills`、`visual_contexts`、
`visual_context_event_links`、`visual_context_comment_links`。
