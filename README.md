...
- 原帖图片转述文字缓存 30 分钟，同楼层并发读取只发起一次上下文请求；活跃楼层缓存默认缩短到 60 秒且新一轮通知会绕过早于该通知的旧缓存，通知已有原帖快照时避免再请求一次原帖；
...
- 图片预处理只使用事件按图片增加的额外回复宽限，并额外设置单事件 60 秒硬上限；Provider 卡住或预算耗尽会立即降级并继续最终 Agent/工具链，无图片事件不增加任何图片 Provider 调用；
...
图片 LLM 预处理共享本事件已经按图片数量增加的回复宽限，并设置单事件最多 60 秒的硬预算。
顺序 Provider fallback 会共同消耗这一预算；任一调用卡住时由 `asyncio.wait_for()` 在剩余预算处
...
- v1.2.14 数据库迁移版本为 **v8**：v7 新增通知跨轮回填状态，v8 增加长期元数据清理所需索引；
