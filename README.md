...
然后使用另一个账号发布一条全新的 @。升级 v1.1.1 后也建议先等待该日志，再开始验证。
...
v1.1.1 的图片能力范围是接收和理解；评论图片上传列为待真实账号验证项。
...
| 覆盖更新后适配器实例为空 | 升级 v1.1.1；插件会在热重载阶段重建已启用实例，日志显示“插件更新后已重新加载小黑盒适配器实例” |
...
| 历史 @ 进入事件记录 | 升级 v1.1.1，等待“通知历史基线已建立”后再发送新 @ |
| 同一消息出现多条回复 | 升级 v1.1.1，检查事件状态和 `outgoing_replies`；`send_unknown` 交由人工核对 |
