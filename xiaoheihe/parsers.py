...
def parse_feed(
    ...
    *,
    ...
    """Normalize the verified recommendation-feed response into stable fields."""
    candidate = payload.get("result", payload.get("data", payload))
    if isinstance(candidate, Mapping):
        body = candidate
        if not any(field in body for field in ("links", "items", "list")):
            raise ResponseShapeError("推荐流响应缺少 links/items/list 字段")
        raw_items = _first(body, "links", "items", "list", default=[])
    elif isinstance(candidate, list):
        body = payload
        raw_items = candidate
    else:
...
def _feed_section_names(post: Mapping[str, Any]) -> list[str]:
    ...
    for field in (
        ...
        for value in values:
            if isinstance(value, Mapping):
                name = str(_first(value, "name", "title", "text", "tag"))
            else:
                name = str(value or "")
            if name:
