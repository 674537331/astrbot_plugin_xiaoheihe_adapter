...
v1.3.0 的真实分区浏览只新增插件自身的小黑盒 HTTP 读取逻辑和 Plugin Page UI，不引入新的 AstrBot 核心 API 依赖，也不改变 QQ 或其他平台行为。
...
| 发送者身份 | 非临时 `extra_user_content_parts` | 每轮昵称 + UID / 备用锚点随用户历史保留 |
...
同一楼层共享 `xhh_thread_<post_id>_<root_comment_id>`，以保留公开讨论连续性。由于 AstrBot 的普通 Conversation 历史本身不保存平台 sender 字段，插件把当前发送者昵称和 UID / 备用锚点作为非临时用户内容块加入该轮历史。

动态帖子正文、楼层、图片描述和焦点信息使用临时内容，只参与当前请求。
...
- **会话 ID 不变**：不会因为浏览来源升级拆分既有 Conversation；
- **主动候选/发送闸门不变**：来源升级不绕过审核与幂等逻辑。
...
另有 CodeQL、Dependency Review 和 Secret Scan。发布版本一致性由 `tools/validate_repository.py` 强制校验，避免 README、CHANGELOG、元数据和运行时诊断再次出现版本漂移。
