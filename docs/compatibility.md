# AstrBot 兼容性说明（v1.2.7）

## 调查范围

项目面向 AstrBot `>=4.24.2,<5`，重点兼容 4.26.2；接收与真实评论链路的最新用户复测环境为
AstrBot 4.26.8。v1.2.7 保持覆盖更新后的平台实例协调、评论与原帖上下文合并，并为多图视觉预处理提供按图片数量计算的回复时间。
2026-08-02 发布 v1.2.7 前再次核对：
...
数据库打开时按 `schema_migrations` 顺序执行增量迁移。v1.2.7 的最新迁移版本为 v6；除旧版
