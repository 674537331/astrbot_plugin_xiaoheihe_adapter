...
**v1.3.3 不新增、删除或修改任何小黑盒 HTTP 端点、参数或响应解析契约。** 本版本只调整送给模型的楼层相关性路由、图片来源优先级和辅助处理时间预算；真实分区、推荐流回退、通知读取与评论写入的网络行为保持 v1.3.x 原契约。
...
v1.3.3 的图片上下文没有新增请求：当帖子/楼层树已解析出直接回复对象、但该树节点恰好缺少媒体字段时，插件可使用**同一条通知已有的 `comment_b` 媒体字段**作为该目标的视觉回退；通知级 `comment_b_id` 与嵌套评论 ID 只要存在就必须全部精确匹配已解析目标，任一冲突都会拒绝该媒体。目标身份和文本仍以已解析回复链为准，这属于本地上下文归并，不改变 HTTP 端点或参数。
