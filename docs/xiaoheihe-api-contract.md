...
原生用户消息；评论图片和原帖图片交替加入 AstrBot `Image` 组件。v1.2.13 的被动楼层会在
`on_llm_request` 阶段再次按图片来源预处理：原帖原图不会继续进入最终 Agent 请求，当前评论
原图仅在视觉预处理全部失败时作为原生兜底保留。
...
主动事件在 `dry_run=false` 且 `review_required=false` 时跳过候选审核，直接调用同一个评论
发送入口。该模式不改变 Workshop 请求路径、表单字段、签名或成功响应解析，并继续复用
事件发送记录、楼层锁、重复发送闸门和发送状态未知核对。
