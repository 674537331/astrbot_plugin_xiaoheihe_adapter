...
- 直接回复对象与楼层锚点的 `comment:<id>` 必须分别精确匹配本轮回复目标与根评论；
...
- 没有原帖视觉输入时，压缩 Provider 无法自行注入 `post_image_summary`；
...
验证脚本会把当前仓库根目录固定为包导入首选路径；即使系统环境里另有同名
`xiaoheihe` 安装，也不会误检其他副本后把当前 checkout 宣告为通过。
