...
fail-closed 路由，而是按固定图片、固定 LLM、当前会话 Provider 生成主背景视觉快照；全部失败
仍把原图交给 AstrBot 主视觉路径。短的“无法加载/看不到图片”占位输出视为失败且不缓存。
图片 LLM 预处理只消费事件已有的图片额外回复宽限，总预算同时受实际图片数、配置图片上限和
单图超时约束，默认 1/2/3/4–6 图为 30/60/90/120 秒，单个 Provider 最多占 60 秒；预算耗尽
立即走上述降级，不继续占用最终 Agent 或工具链的回复时间。无图片事件不创建该预处理调用链。
