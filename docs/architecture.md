...
小黑盒 HTTP API
  ...
  → 图片来源路由（当前评论图片 > 原帖图片；固定图片 Provider 可分组压缩）
...
图片 URL 在 `ContextBuilder` 收集阶段同时记录 `current_comment` / `original_post` 来源。配置固定
图片 Provider 时两类图片分组理解：被动回复中的当前评论图片保持最高相关性，原帖图片使用较短
的低优先级描述；任意图片分组处理失败时保留全部原图并交回 AstrBot 原生视觉链路。没有固定
图片 Provider 时不增加视觉 LLM 调用，只给原图追加不含社区内容的可信来源映射。主动推荐流不
使用被动楼层文本压缩，原帖文字和图片继续作为主要内容。
