...
- `context_builder.py` / `context_compression.py` / `context_relevance.py`：帖子/楼层上下文、焦点路由、身份锚点、长楼层压缩、原帖相关性和图片来源距离。
...
```text
related / partial / unclear
  → 保留原帖文字与允许使用的原帖视觉背景

drifted
  → 当前消息 + 直接回复对象 + 当前楼层成为主上下文
  → 省略压缩结果中的原帖标题/摘要/图片摘要
  → 视觉链不再注入原帖图片或原帖视觉缓存

当前消息显式引用原帖 / 主帖 / 楼主 / 帖子内容 / 原帖图片
  → 无条件恢复原帖背景
```
...
长楼层压缩的当前消息和直接回复对象始终保留原文。前置阶段已经尝试过压缩后，同一事件后续 Agent 注入不会因失败再次请求慢 Provider，而是回到有界确定性窗口。
...
图片槽位先为每个近距离来源保留代表图，再加入同来源的额外图片；原帖图永远位于本地回复链图片之后。因此“别人发图 → 用户回复该评论并 @Bot”时，被直接回复的图片不会被主贴图片挤掉。若帖子/楼层树已经找到直接回复对象但该节点缺少媒体字段，插件会仅对**同一已解析目标**回退通知中的 `comment_b` 媒体；通知级 `comment_b_id` 与嵌套评论 ID 只要存在就必须全部精确匹配目标，任一冲突都拒绝回退。楼层树仍优先承担身份/文本归属。直接回复对象和楼层锚点图片各自保留 `comment:<id>` 身份键；来源、角色或身份键不一致时降级为未知来源，不把外部字段直接当成可信所有权。

插件只保存安全公开 HTTPS URL 的有限引用；成功识图后向最终 Agent 提供来源明确的文字描述。图片所有者的昵称、UID、身份角色与本地锚点仍作为程序级归属数据参与所有权判断，但 v1.3.2 起明确禁止图片压缩 Provider 把这些身份值写入视觉摘要。只有所有者 UID 与当前发送者 UID 完全一致时才允许称为“你发的图片”；UID 或来源无法确认时继续 fail-closed。

全部候选失败、超时或返回占位结果时移除原图并注入可信失败说明，禁止最终模型猜图。当前评论和直接回复对象属于最近视觉来源，可在图片预处理全部失败时保留为 AstrBot 原生视觉最后兜底；楼层锚点、原帖和未知来源不获得这一高优先级兜底。主动帖子成功识图可生成 24 小时视觉文字快照。内存 LRU 和 SQLite 都只保存文字描述、图片指纹和脱敏元数据，不保存图片字节。
...
- v1.3.3 为外层回复 timeout 分离预留 context / vision / main / provider-fallback 预算，视觉预算计算会扣除 context 预留；上下文压缩失败不会在同一事件重复请求慢 Provider；
- v1.3.3 **没有新增数据库迁移、配置项或运行依赖**。
