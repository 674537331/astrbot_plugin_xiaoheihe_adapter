...
并在 `on_llm_request` 中额外追加非临时 `<xiaoheihe_sender_identity>` 内容块，昵称与 UID 作为
...
消息、原帖、每条楼层评论、直接回复对象及每张图片生成“昵称 + UID”；图片归属与 URL 按相同
索引传递，长度错位、来源冲突或 UID 冲突时降级为未知归属。长楼层压缩器只可从本地参与者白名单
选择 `speaker`，本地解析器丢弃任何编造身份，原帖摘要和图片摘要在渲染时重新附上原帖作者。
...
都校验当前帖子作者。迁移前没有身份的旧快照、跨帖子同图指纹以及作者 UID 不匹配的记录均不复用，
随后按正常识图链重新生成。新增字段是有界短文本，LRU 条数、24 小时 TTL 和不保存图片字节的边界
保持不变。
